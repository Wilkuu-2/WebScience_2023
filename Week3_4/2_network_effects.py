...
def get_all_song_derivatives(timelines: Dict[str, Dict[datetime, Any]]) -> Dict[str, Dict[datetime, float]]: 
    derivatives = {}  
...
def average_daily(derivative: Dict[datetime, float]) -> float: 
    N = 0
    sum = 0
...
def median_daily(derivative: Dict[datetime, float]) -> np.floating[Any]:
...
if __name__ == "__main__": 
    ...
    graph_median_change_per_day = [median_daily(derivative) for derivative in derivatives.values()]
    graph_max_viewcount = [max_viewcounts(timeline) for timeline in timelines.values()]
