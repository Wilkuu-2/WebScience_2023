...
while len(random_song_ids_filtered) < TEST_COUNT:
    ...
    if random_id not in map(lambda song: song["id"], res2[0].json):
        if random_id not in map(lambda song: song["id"], res3[0].json):
            random_song_ids_filtered.append(random_id)
...
# Test if every song is in the top 100 in every data point
for song_id in random_song_ids:
    print("TEST", song_id, "every data point:", len(get_all_song_data_points(song_id)) == len(res))
